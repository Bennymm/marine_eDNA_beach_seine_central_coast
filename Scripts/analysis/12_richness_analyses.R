...
mm05 <- gls(rich_diff ~ silt_percent,
            ...
            data = data1)
#mixed
mm06 <- gls(rich_diff ~ h100m + h1000m +
              silt_percent, 
            correlation = corSymm(form = ~1|site),
            data = data1)
...
mm06_std <- gls(rich_diff ~ h100m + h1000m +
              silt_percent, 
            correlation = corSymm(form = ~1|site),
            data = data2)
coefficients(mm06_std)

confint <- as.data.frame(confint(mm06_std))
ceof <- as.data.frame(mm06_std$coefficients)
